...
export const Card = ({ className, children, addDividers, showCloseButton, style, close }) => {
    const isRoutesType = (child) => typeof child === 'object' && child?.type?.name === 'Routes';

    return (
        <div style={style} className={["card border-0 rounded-1", className ? className : ""].join(" ").trim().replace(/\s+/g, " ")} >
            {showCloseButton && (
